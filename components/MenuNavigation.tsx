...
export default function MenuNavigation({ activeSection, onSectionChange }: MenuNavigationProps) {
  return (
    <motion.nav
      className="fixed top-0 right-0 z-50 p-6 md:p-12"
        ...
        {navItems.map((item, index) => (
          <motion.button
            ...
            className={`transition-colors duration-300 font-light relative group ${
              activeSection === item.id ? 'text-foreground' : 'text-foreground/40 hover:text-foreground/70 active:text-foreground'
            }`}
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            <span className="relative">
              ...
              {activeSection === item.id && (
                <motion.span
                  className="absolute -left-2 sm:-left-3 top-1/2 -translate-y-1/2 w-1 sm:w-1.5 h-1 sm:h-1.5 bg-accent"
